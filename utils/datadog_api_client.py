...
Used by the profiler for schema discovery and for backfilling gaps.
"""
...
class DatadogClient:
    ...
    def query_metrics(
        ...
        """Execute a DD query and return a standard otel_etl DataFrame.
        ...
        Returns:
            DataFrame(timestamp, metric, labels, value).
        """
        from datadog_api_client import ApiClient
        from datadog_api_client.v1.api.metrics_api import MetricsApi
        ...
        for series in (response.series or []):
            ...
            normalized_name = normalize_dd_metric_name(raw_metric)

            # Parse tags
            tags_dict = parse_dd_tags(series.tag_set or [])
            labels = map_dd_tags_to_otel(tags_dict)
    ...
    def query_metrics_range(
        ...
        Convenience method that builds queries for each metric name.
        ...
        Returns:
            Combined DataFrame(timestamp, metric, labels, value).
        """
        frames = []
